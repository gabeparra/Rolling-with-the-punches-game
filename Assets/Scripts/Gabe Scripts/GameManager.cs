...
public class GameManager : MonoBehaviour
{
    ...
    public float GetReloadMultiplier()
    {
        ...
        {
            if (upgrade != null && upgrade.type == Upgrade.UpgradeType.ReloadSpeed && HasUpgrade(upgrade.id))
                mult -= upgrade.value;
        }
    ...
    public float GetDamageMultiplier()
    {
        foreach (var upgrade in allUpgrades)
        {
            if (upgrade != null && upgrade.type == Upgrade.UpgradeType.Damage && HasUpgrade(upgrade.id))
    ...
    public int GetMaxAmmoBonus()
    {
        ...
        foreach (var upgrade in allUpgrades)
        {
            if (upgrade != null && upgrade.type == Upgrade.UpgradeType.MaxAmmo && HasUpgrade(upgrade.id))
